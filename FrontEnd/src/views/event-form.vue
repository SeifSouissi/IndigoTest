...
export default {
    ...
    methods: {
        ...
        checkDates: function(evt){
           var startDate = new Date(evt.dateTime);
           var eventStartDate = new Date("2019-10-11T00:00:00-07:00");
           var endDate = new Date(evt.dateTime );
           endDate.setMinutes(endDate.getMinutes() + parseInt(evt.duration));
           var eventEndDate = new Date("2019-10-14T00:00:00-07:00");
        ...
        dispatchChange: function( evt, type = 'createEvent' ) {
            ...
            if (!this.checkDates(this.thisEvent)) {
                const message =  'Please select a valid date between the start and end of the event date';
                this.$toasted.show(
                  message,
                  {
                    position: 'top-left',
